...
file_dist_glob_phenotypes <- './input_tables/GlobalDistance_Phenotypes/file_dist_glob_phenotypes.rds'

readRDS(file_dist_glob_phenotypes) -> file_dist_glob_phenotypes
...
ProxPath_Phenotypes_parse_distance_table <- function (
    distance_file = file_dist_glob_phenotypes, ## output of
    expansion = TRUE,
    background = all.signor.id,
    default_min=0.1,
    statistic_zcore = 'mean',
    zcore_threshold= -1.96,
    save_directory){
  ...
  if (expansion == TRUE){
    ...
    distance_file$EndPathways <- sapply(strsplit(as.character(distance_file$EndPathways ), ":"),"[", 1)   ## "[" lascialo sempre, 1 ti dice quale elemento ci serve        
  ...
  table.input$Effect[ table.input$Final_Effect == 1] <- 'up-regulates'
  
  table.input$Effect[ table.input$Final_Effect == 0] <- '-'
  
  table.input$Effect[ table.input$Final_Effect == -1] <- 'down-regulates'
  
  table.input$Effect[ table.input$Path_Length == 0] <- 'up-regulates'
...
ProxPath_Phenotypes_enrichment <- function (
    analysis_name,
  ...
  analysis_name=paste0(analysis_name,'_to_Phenotype')
  ...
  output.table.global.stat %>%
    filter( pvalue < pvalue_threshold & 
              Effect != '-' ) -> output.table.global.stat
  ...
  if (save_tables == TRUE){ 
    ...
    for (list_i in input_list){
      ...
      table.zscore.filtered.slim%>%
        filter(QueryNode %in% USER_dataset$QueryNode & EndPathways %in% output.table.global.stat_filtered$EndPathways)-> USER.Phen.Paths
      
      if (nrow(USER.Phen.Paths) == 0){
        print(paste0('Warning, check that gene names are provided as first column in input, id are not going to be saved...'))
      }
      ...
      # EndPathways = tolower(gsub('_', ' ', EndPathways)
        
      ##PLOT UP_REGULATES           
      output.table.global.stat_filtered%>%
        filter(Effect == 'up-regulates')%>%
        arrange(as.integer(Log10_p_value_plot))%>%
        mutate(EndPathways = tolower(gsub('_', ' ', EndPathways)))%>%
        as_tibble() -> output.table.global.stat.general.plot.up #%>% distinct(EndPathways, .keep_all = T)
      
      output.table.global.stat.general.plot.up$EndPathways<- factor(output.table.global.stat.general.plot.up$EndPathways, 
                                                                 levels= output.table.global.stat.general.plot.up$EndPathways)
      
      ggplot(output.table.global.stat.general.plot.up, aes(x = factor(EndPathways), y = Log10_p_value_plot, fill = Effect)) +
        # geom_bar( stat = 'identity')+
        geom_col(position = "dodge")+
        ggtitle(paste0('Significantly close up-regulated phenotypes - ' , list_i)) +
        ylab("-Log10(p-value)") + xlab('phenotype') +
        
        theme_bw()+
        scale_fill_manual(values = c("#0C838C"))+
      ...
      ##PLOT DOWN_REGULATES           
      output.table.global.stat_filtered%>%
        filter(Effect == 'down-regulates')%>%
        arrange(as.integer(Log10_p_value_plot))%>%
        mutate(EndPathways = tolower(gsub('_', ' ', EndPathways)))%>%
        as_tibble() -> output.table.global.stat.general.plot.down #%>% distinct(EndPathways, .keep_all = T)
      
      output.table.global.stat.general.plot.down$EndPathways<- factor(output.table.global.stat.general.plot.down$EndPathways, 
                                                                 levels= output.table.global.stat.general.plot.down$EndPathways)
      
      ggplot(output.table.global.stat.general.plot.down, aes(x = factor(EndPathways), y = Log10_p_value_plot, fill = Effect)) +
        # geom_bar( stat = 'identity')+
        geom_col(position = "dodge")+
        ggtitle(paste0('Significantly close down-regulated phenotypes - ' , list_i)) +
        ylab("-Log10(p-value)") + xlab('phenotype') +
        theme_bw()+
        scale_fill_manual(values = c("#9DD2C7"))+
        geom_text(aes(label = hits_USER), colour = "black", size = 2, hjust= 1, position = position_dodge(.9))+
        coord_flip()->plot_1
      plot_1
      plot_list[[i]]<- plot_1
      ...
      ggsave(paste0(save_directory, analysis_name, '_plot.pdf'), device= 'pdf', plot = combined_plots, height = 15, width =20, units = 'cm')
    }
